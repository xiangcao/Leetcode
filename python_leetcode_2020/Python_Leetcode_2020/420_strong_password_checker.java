...
class Solution {
    ...
    public int strongPasswordChecker(String s) {
        ...
        if (arr.length < 6) {
            ...
            res += over_len;
            //int k = 1;  //test case "aaaabbaaabbaaa123456A";  "aaaabbaaabbaaa123456A"
            //int k = 2; //test case "aaaaabbbb1234567890ABA"
            for (int k = 1; k < 3; k++) {
            ...
            //after the loop above, all repeative elements will appear 3*m+2 times
            for (int i = 0; i < arr.length; i++) {
                if (arr[i] >= 3 && over_len > 0) {
                    int need = arr[i] - 2;
                    arr[i] -= Math.min(over_len, need);
                    over_len -= need;
                }

                if (arr[i] >= 3) left_over += arr[i] / 3; //replacement needed to fix 3 repeative elements;
            }
...
For any repeating sequences with len % 3 == 0, we can reduce one replacement by deleting one character. For any repeating sequences with len % 3 == 1, we can reduce one replacement by deleting two character. For the remaining sequences, we can reduce every replacement by deleting three character.
    ...
    def strongPasswordChecker(self, password: str) -> int:
        ...
        if len(s) < 6: #Insert only;
            return max(missing_type, 6 - len(s))
        elif len(s) <= 20: # Replace only
